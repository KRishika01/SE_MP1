...
/**
 * An index operation that writes to the index.
...
public abstract class WriteToIndexOperation extends IndexOperation {
     ...
     */
    public WriteToIndexOperation(IndexResourceProvider resourceProvider) {
    ...
    public void run() {
        try {
            // Use interface method to get write lock
            resourceProvider.getReadWriteLock().writeLock().lock();
            logger.debug("Starting search index operation");
            doRun();
            logger.debug("Search index operation complete");
            ...
            resourceProvider.getReadWriteLock().writeLock().unlock();
        }
        // Reset reader after write so subsequent reads see the changes
        resourceProvider.resetSharedReader();
    }
}